...
class CustomSelector<T> extends AbstractSelector<T> {
    private final Predicate<Context<?>> predicate;

    public CustomSelector(Selector<?> prev, Predicate<Context<?>> predicate) {
        super(prev);
    ...
    public boolean test(Context<?> context) {
        return context != null && this.predicate.test(context) && super.test(context);
    }
}
